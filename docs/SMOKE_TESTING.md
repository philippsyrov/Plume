...
| B17 | With Browser visible, open Settings, Help, Workspace views, and one session dialog in turn. Close each before opening the next. | Every overlay waits for the native page to suspend, appears fully above it, and returns to the same Browser tabs and live page state after close. The deterministic rejected and never-resolving suspension paths remain unit-test evidence rather than packaged fault-injection smoke. |
| B18 | Persist a wide split Browser layout, quit Plume, relaunch at a narrower window width, and reopen that chat's Browser. | The split descriptor is normalized to the narrower measured canvas: chat and Browser remain usable, the divider stays in bounds, and the corrected width persists for the next relaunch. |
