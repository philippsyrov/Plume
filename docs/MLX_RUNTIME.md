...
Current implementation and upstream notes for Plume-owned MLX-LM and MLX-VLM. Provider
...
- Release packaging generates a relocatable arm64 runtime with pinned CPython
  3.12.13, `mlx-lm` 0.31.3, `mlx-vlm` 0.5.0, `mlx` 0.32.0, and `mlx-metal`
  0.32.0. Its lock,
...
- The runtime is not a model. Qwen Coder 1.5B and Qwen2-VL 2B weights are
  separate explicit, pinned, verified, resumable downloads into Application
  Support.
- Fixed catalog Qwen Coder and Qwen2-VL start without a project. Arbitrary discovered folders
...
src-tauri/src/providers/
  ...
  catalog.rs             fixed Qwen Coder/Qwen2-VL identity + receipt validation
...
trusted project. `providers.catalogStart({ catalogId })` launches only a fixed
receipt-backed Qwen Coder or Qwen2-VL catalog model and has no project input. Both enter
...
The packaged release includes its own pinned MLX-LM and MLX-VLM runtime. That build-time
resource is generated by the explicit release/smoke preparation command; Plume
does not install Python packages at runtime. Fixed Qwen Coder and Qwen2-VL weights remain
separate user-triggered network actions with visible progress, cancel, resume,
