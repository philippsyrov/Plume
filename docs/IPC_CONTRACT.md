...
| --------------- | -------- | ------------ | ---------------------------------------------- |
| `ChatStreamId`  | `string` | backend      | until `chat.done` fires for that id; further `chat.cancel(id)` is a no-op |
...
`seq` field. Gaps in `seq` mark the stream corrupt per
...
1. IPC version.
2. Payload shape: non-empty `providerId`, non-empty `modelId`,
   ...
   returned synchronously from `chat.send` — before any stream id
   is minted.
3. Provider boundary: `providerId !== 'ollama'` → `BadArgument`.
4. Stream-start: `chat.send` returns `{ streamId, … }` as soon as
...
**Subscribe-before-await.** Tauri events are not replayed. The
backend defers emitting any `chat.token` event until after
`chat.send` has returned, so the frontend can safely
`await startChatStream({...})` and then `listen()`. Subscribing
before sending also works and is the conservative pattern.
