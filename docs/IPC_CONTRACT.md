...
| --------------- | -------- | ------------ | ---------------------------------------------- |
| `ChatStreamId`  | `string` | backend      | reserved for D7.1 streaming `chat.send`; not minted today |
...
D7 ships the synchronous, non-streaming subset of chat. The streaming
variant (`chat.send → ChatStreamId` plus `chat.token` / `chat.done` /
`chat.tool` events plus `chat.cancel`) is queued as D7.1 — see
`docs/IPC_ROADMAP.md § Chat streaming` for the reserved shape. The
verb names are kept identical because the request payload is
forwards-compatible: a future `stream: true` flag will opt in to the
streaming shape without renaming the command.

```
chat.send(req: ChatSendPayload)   -> ChatResponse
...
type ChatMessage = {
  role: 'system' | 'user' | 'assistant' | 'tool'; // 'tool' rejected with BadArgument in D7
  content: string;
};
...
type ChatResponse = {
  ...
  modelId: string;                               // model id the runtime reports it served
  durationMs: number;                            // backend-measured wall-clock
  finish: 'stop' | 'length';                     // 'length' surfaces an Ollama done:false
};
```

`chat.send` is the only chat-shaped verb today. It is **synchronous**:
the call resolves with the full assistant message at once. There is no
stream id, no events, no cancel verb in this slice. Callers must be
prepared to wait — generation can take many seconds on modest
hardware, and the backend uses a 5-minute timeout on the underlying
HTTP read.
...
- No prompt assembly. The user instruction is forwarded as the
  ...
  the propose-diff slice; D7's `chat.send` does not read files.
- No `mode`, `attachments`, or session policy fields. They are
  reserved in the streaming shape and ship when the propose-diff /
  scoped-edit modes do.
- No streaming, no token events, no cancellation.
...
4. Transport: connect, send, parse. Failures map to:
   - `BadArgument` — Ollama answered 404 (model not in `/api/tags`)
     or any other 4xx.
   - `ProviderDown { provider: 'ollama', reason }` — could not reach
     the daemon, or Ollama returned 5xx.
   - `Internal` — Ollama answered with a body that did not parse.
