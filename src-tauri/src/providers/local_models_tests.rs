//! Tests for `local_models`. Split into a sibling file via
//! `#[path]` (D76) so the production module stays under the
//! decomposition cap.
use super::*;
use std::fs;
...
fn external_source_symlinks_are_skipped() {
    ...
    symlink(&outside_target, cache_td.path().join("link.gguf")).expect("create symlink in cache");
